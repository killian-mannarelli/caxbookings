        ...
        if room_id is not None:
            ...
            if time_span_start is not None and time_span_end is not None:
                for (computer) in queryset:
                    ...
                    bookings = Bookings.objects.filter(computer=computer.id, start__gte=parser.parse(
                        time_span_start), end__lte=parser.parse(time_span_end), status=1)
                    if(bookings.count() > 0):
...
def get_room_current_capacity(room_id, time_start, time_end):
    ...
    for computer in computers:
        bookings = Bookings.objects.filter(
            computer=computer, start__gte=time_start, end__lte=time_end, status=1)
        if len(bookings) > 0:
